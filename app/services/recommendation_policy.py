...
"""출력 스키마가 모델에게 요구하는 길이(``minLength``). 폐기선과 **독립된** 값입니다.

[현재 상태] 산문 프롬프트(``prompt._MESSAGE_RULES``)에서 길이·문장 수 요구가
빠졌고, 지금 이 숫자를 싣는 곳은 ``build_*_schema()`` 의 ``minLength`` 뿐입니다.
그것도 구조화 출력에는 못 실려(``bedrock_client.UNSUPPORTED_SCHEMA_KEYWORDS``)
``schema_instruction`` 이 프롬프트 텍스트로 넣는 경로로만 모델에 닿습니다.
아래 실측 기록은 산문이 길이와 문장 수를 함께 요구하던 때의 것이라, 지금 조건에서
그대로 재현되지 않습니다. 길이를 다시 손볼 때는 실측부터 새로 잡으세요.
...
실제 지렛대는 문장 수였습니다. 같은 표본에서 4문장은 109~143자, 5문장은 138·146자로
갈렸습니다. 그 시기에는 prompt.py 가 문장 수를 함께 요구했습니다 — 지금은 아닙니다.
"""
