...
# 카테고리를 백엔드 목록(다섯 개)에 맞추면서 접힌 카테고리의 핵심어를 흡수시켰습니다.
# 여기를 같이 옮기지 않으면 검증이 통째로 무너집니다 — 아래 함수는 **알려진 카테고리면
# 씨앗을 무시하고 카테고리 핵심어만** 봅니다. 디저트가 "스페셜티 드립백 세트" 로 검색해도
# 커피 핵심어가 이 표에 없으면 결과가 전부 탈락합니다.
_CATEGORY_KEYWORDS: dict[str, tuple[str, ...]] = {
    "상품권": (
        "상품권", "금액권", "교환권", "이용권", "쿠폰", "기프트카드",
        "기프티콘", "e카드", "모바일교환권", "기프트콘", "관람권",
    ),
    "디저트": (
    ...
    "패션·잡화": (
        "패션", "지갑", "가방", "파우치", "액세서리", "잡화",
        # ↓ 뷰티·화장품에서 넘어온 핵심어
        "화장품", "뷰티", "립밤", "립스틱", "향수", "스킨", "로션", "핸드크림", "메이크업",
    ),
    "생활용품": (
        "생활", "타월", "수건", "텀블러", "식기", "주방", "세제",
        # ↓ 디지털 액세서리에서 넘어온 핵심어. 씨앗 "휴대폰 거치대" 가 여기에 의존합니다.
        "충전", "케이블", "거치대", "이어폰", "보조배터리", "디지털",
    ),
...
def build_query(category: str, example: str | None, low: int, high: int) -> str:
    ...
    카테고리명("디저트")만으로는 검색이 잘 되지 않아, 정책이 정한 구체적인
