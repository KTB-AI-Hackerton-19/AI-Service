...
class TestTheEngineDispatch:
    ...
    async def test_the_graph_is_the_default_path(self):
        """그래프가 기본 경로입니다. 끄면 분할 경로로 내려갑니다."""
        assert type(settings).model_fields["recommendation_langgraph"].default is True

    async def test_a_missing_langgraph_falls_back_instead_of_failing(
        self, stages, monkeypatch
    ):
        """langgraph 가 없는 환경에서도 추천은 나가야 합니다.

        기본값이 켜져 있으므로 이 폴백이 이제 배포 안전망입니다. 임포트가 실패하면
        경고를 남기고 분할 경로로 내려갑니다.
        """
        import builtins
