...
| 검색·목록 페이지의 숫자는 그 상품 가격이 아님 | `kind: listing` 은 가격을 비움 |
| 검색어 가격 힌트를 상한으로 주면 비싼 것만 나옴 | 범위 중앙값 사용 |

**한계**: Tavily 는 쇼핑 API 가 아니라 범용 웹 검색이라 가격대로 결과를 거를 수 없습니다.
가격은 사후에 확인되므로, 범위 안을 우선 정렬하되 버리지는 않습니다. 버리면 결과가 0건이 되는
경우가 많기 때문입니다. 범위를 벗어난 상품은 `rationale.warnings` 와 상품별 `reason` 에 명시됩니다.
