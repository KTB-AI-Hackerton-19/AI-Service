...
각 최상위 작업에는 `REQUEST_TIMEOUT_SECONDS` 제한 시간이 걸립니다.
...
| `TAVILY_EXTRACT_TIMEOUT_SECONDS` | Extract 묶음 제한 시간(초) | `10` |
...
3. **추출** — vLLM 은 `response_format: json_schema` 로 기록 배열을 받습니다. 구조화 출력을
   지원하지 않는 Bedrock 은 같은 스키마를 프롬프트에 실어 받습니다.
...
검색 키가 없거나 관련 상세상품이 없거나 검색이 실패하면 `products`는 빈 배열이 되고
...
출력은 통과한 번호 배열(`{"keep": [1, 2, 5]}`)로 받습니다. 항목마다 `{n, keep}` 객체를 받으면
12건 기준 141토큰인데 번호 배열은 18토큰이고 그만큼 생성 시간도 짧습니다(2.2초 → 1.7초).
