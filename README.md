...
추천 상품은 현재 **실제 상품이 아니라 상품 '유형'**입니다(`recommendation_policy.SAFE_EXAMPLES`).
기획안의 리스크 대응이 "실시간 쇼핑몰 전체를 검색하지 않는다"로 잡혀 있어 환각을 막는 쪽을 택했습니다.
기획안이 함께 요구하는 "구매 페이지로 이동"까지 채우려면 상품 카탈로그에 링크를 넣거나
쇼핑 검색을 붙여야 하며, 이는 추천 담당자와 정할 사안입니다.
